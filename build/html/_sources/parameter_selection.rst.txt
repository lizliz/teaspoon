.. _parameter_selection:

Parameter selection module documentation
=======================================================

This page provides a summary of the Python functions used in "`On the Automatic Parameter Selection for Permutation Entropy <https://arxiv.org/abs/1905.06443>`_" for automatically finding an appropriate dimension and delay for both delay embedding (i.e. Takens' embedding or state space reconstruction) and permutations for permutation entropy or ordinal partition networks. To use this module, the following python packages are needed:

* numpy
* scipy
* pyentrp
* matplotlib
* pandas

The following is the table of contents for the parameter selection methods:

.. toctree::
   :maxdepth: 2
   
   Mutual Information <MI.rst>
   Auto-correlation <AC.rst>
   Fourier Spectrum Analysis <FSA.rst>
   Permutation Auto Mutual Information <PAMI.rst>
   Multi-scale Permutation Entropy <MsPE.rst>
   False Nearest Neighbors <FNN.rst>









