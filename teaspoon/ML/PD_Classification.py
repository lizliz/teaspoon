...
def train_test_split(DgmsDF,labels_col,dgm_col,params,*args):
    ...
    *args : 
    ...
    if params.TF_Learning:
        ...
        skf= KFold(n_splits=params.k_fold_cv, shuffle =True, random_state=params.seed) # set a constant random_state number to be able to get same train-test for all classifiers
        ...
        for k_fold in range(params.k_fold_cv):
            if type(dgm_col)==list:
                if params.feature_function != fF.tent and params.feature_function != fF.interp_polynomial:
                    raise Exception("The list format for dgm_col is only valid for template functions.")
                # user may enter a list of diagram label
                DgmsDF = DgmsDF.sort_index()
                D_train_train, D_train_test = DgmsDF.iloc[indices_train_train[k_fold]], DgmsDF.iloc[indices_train_test[k_fold]]
                ...
                D_test_train, D_test_test = DgmsDF_test.iloc[indices_test_train[k_fold]], DgmsDF_test.iloc[indices_test_test[k_fold]]
                L_test_train, L_test_test = DgmsDF_test[labels_col][indices_test_train[k_fold]], DgmsDF_test[labels_col][indices_test_test[k_fold]]
                
            else:
                DgmsDF = DgmsDF.sort_index()
                # training and test set for training set diagrams/landscapes/images 
                D_train_train, D_train_test = DgmsDF[dgm_col][indices_train_train[k_fold]], DgmsDF[dgm_col][indices_train_test[k_fold]]
                L_train_train, L_train_test = DgmsDF[labels_col][indices_train_train[k_fold]], DgmsDF[labels_col][indices_train_test[k_fold]]  
                
                DgmsDF_test = DgmsDF_test.sort_index()
                # training and test set for test set  diagrams/landscapes/images     
                D_test_train, D_test_test = DgmsDF_test[dgm_col][indices_test_train[k_fold]], DgmsDF_test[dgm_col][indices_test_test[k_fold]]
                L_test_train, L_test_test = DgmsDF_test[labels_col][indices_test_train[k_fold]], DgmsDF_test[labels_col][indices_test_test[k_fold]]             
        ...
        for k_fold in range(params.k_fold_cv):
            if type(dgm_col)==list:
                ...
                # user may enter a list of diagram label
                DgmsDF = DgmsDF.sort_index()
                D_train, D_test = DgmsDF.iloc[indices_train[k_fold]], DgmsDF.iloc[indices_test[k_fold]]
                L_train, L_test = D_train[labels_col], D_test[labels_col]
            else:
                DgmsDF = DgmsDF.sort_index()
                # training and test set for training set diagrams/landscapes/images 
                D_train, D_test = DgmsDF[dgm_col][indices_train[k_fold]], DgmsDF[dgm_col][indices_test[k_fold]]
                L_train, L_test = DgmsDF[labels_col][indices_train[k_fold]], DgmsDF[labels_col][indices_test[k_fold]]         
...
def getPercentScore(DgmsDF,
    ...
    **kwargs : 
    ...
    if params.TF_Learning:    
        ...
        training_set_dgm,test_set_dgm,training_set_label,test_set_label = train_test_split(DgmsDF,labels_col,dgm_col,params,DgmsDF_test)
    else:
        training_set_dgm,test_set_dgm,training_set_label,test_set_label = train_test_split(DgmsDF,labels_col,dgm_col,params)
    ...
    for k_fold in range(params.k_fold_cv):
        ...
        if params.feature_function == fF.tent or params.feature_function == fF.interp_polynomial:
            ...
            G_train = np.concatenate(listOfG_train, axis=1) # feature matrix for training set
            ...
            G_test = np.concatenate(listOfG_test, axis=1) # feature matrix for test set
        ...
        if params.feature_function==fF.F_Landscape:
            ...
            if params.PL_Number is None:
                raise Exception("Please provide the landscape number that you want to extract features from.")
            LN = params.PL_Number
        ...
        ########------- PERSISTENCE IMAGES---------------#########        
            ...
            output = fF.F_Image(D_train, params.pixel_size, params.var, False, [], pers_imager = None,training=True)
            X_train = output['F_Matrix']
            pers_imager = output['pers_imager']
            # use these bounds to compute persistence images of test set
            output = fF.F_Image(D_test, params.pixel_size, params.var, False, [], pers_imager = pers_imager,training=False)
            X_test = output['F_Matrix']

        ########------- CARLSSON COORDINATES---------------#########
        elif params.feature_function == fF.F_CCoordinates:
            if params.FN>5:
                raise Exception("There are five coordinates available currenly. Please enter a number between 1 and 5.")
            
            X_train, TotalNumComb, CombList = fF.F_CCoordinates(D_train, params.FN)
            X_test, TotalNumComb, CombList = fF.F_CCoordinates(D_test, params.FN)
        ...
        ########------- PATH SIGNATURES ---------------#########    
            ...
            if params.L_number == None:
                raise Exception("Please pass the landscape number that will be used in feature extraction")
        ...
        ########------- KERNEL METHODS---------------#########        
            ...
            if params.TF_Learning:
                raise Exception("Transfer learning for Kernel Method is not available. Please try another featurization approach.")
            else:
                if params.sigma is None:
                    raise Exception("Please provide the sigma variable for kernel computation.")
                
                if params.param_tuning:
                    raise Exception("Parameter tuning is not available for kernel methods.")    
                ...
                for i in range(len(poss_comb)):
                    ...
                    KernelTrain[i] = fF.KernelMethod(perDgm2, perDgm1, params.sigma)
                ...
                for i in range(0, N1):
                    perdgm1 = D_train[i]
                    KernelTrain[i, i] = fF.KernelMethod(perdgm1, perdgm1, params.sigma)
                ...
                for i in range(0, N2):
                    ...
                    for k in range(0, N1):
                        perDgm2 = D_train[k]
                        KernelTest[i, k] = fF.KernelMethod(perDgm1, perDgm2, params.sigma) 
                ...
                cr_test = classification_report(L_test, predicted_labels_test,output_dict=True)
                cr_train = classification_report(L_train, predicted_labels_train,output_dict=True)
                ...
                print('Test set acc.: {:.3f} \nTraining set acc.: {:.3f}'.format(cr_test['accuracy'],cr_train['accuracy']))
                print('------------------------------')                     
        ...
        if params.feature_function != fF.KernelMethod:
            ...
            if params.param_tuning:
                if params.parToTune == None:
                    raise Exception("Please pass the parameters to tune using ParameterBucket object.")
                
                # number of features
                n_feat = len(X_train[0,:])
                # concantenate the labels and the train set
                X = np.concatenate((X_train,np.reshape(L_train,(len(L_train),1))), axis=1)
    ...
    print("\nTest Set \n---------")
    print("Average accuracy: {:.3f}\nStandard deviation: {:.3f}".format(np.average(accuracy_test),np.std(accuracy_test)))   
    print("\nTraining Set \n---------")
    print("Average accuracy: {:.3f}\nStandard deviation: {:.3f}".format(np.average(accuracy_train), np.std(accuracy_train))) 
    print("\nFor more metrics, see the outputs.")
    # after all runs are completed, save results depending on user choice
    if saving:
        if saving_path is None:
            raise Exception('Please provide saving path to save classification reports.')
