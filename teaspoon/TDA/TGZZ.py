...
def sliding_windows(window_width, overlap_ratio, time_range):
    ...
    OR = overlap_ratio #ranges from no overlap with 0 to 1 with complete overlap between adjacent windows.
...
def graph_snapshots(edges, I, windows):
    ...
    for window in windows: #go through each window which is representative of a graph snapshot
        ...
        for i, intervals_edge in enumerate(I): #go through each edges intervals
...
def simplicial_complex_representation(TG, G_snapshots, windows, K = 1):
    ...
    for s in S_full:
        T.append([])
    status_vector_prev = [0]*len(S_full) #simplex status vector from previous graph
    ell = len(G_snapshots)
    for i in range(0,ell): #go through each graph
        Gi = G_snapshots[i]
        Di = nx.floyd_warshall_numpy(Gi) #distance matrix using unweighted shortest path
        rips_complex = gudhi.RipsComplex(distance_matrix=Di, max_edge_length=K)
        simplex_tree = rips_complex.create_simplex_tree(max_dimension=K-1)
        S = []
        for filtered_value in simplex_tree.get_filtration():
            S.append(tuple(filtered_value[0]))
        status_vector = [0]*len(S_full) #simplex status vector for when simplices are on.
        for s in S: #go through each simplex in simplices of graph
            s_index = S_full.index(s) #get index in S_full where s is.
            status_vector[s_index] = 1 #set simplex as on if it is in simplicial complex

        for s_index in range(len(status_vector)):
            if status_vector[s_index] == 1 and status_vector_prev[s_index] == 0: #if the simplex appears
                T[s_index].append(t_Gs[i])
            if status_vector[s_index] == 0 and status_vector_prev[s_index] == 1: #if the simplex dissapeared.
                T[s_index].append(t_Gs[i])
...
def plot_persistence_diagram(dgms, windows, dimensions = [0,1], FS = 18):
    ...
    for i,dgm in enumerate(dgms):
        ...
        for j, p in enumerate(dgm):
            ...
            if i in dimensions:
                if j == 0:
                    plt.plot([pair[0]], [pair[1]], plot_markers[i], markersize = MS, label = "$H_"+str(i)+"$", alpha = 0.5)
                else:
                    plt.plot([pair[0]], [pair[1]], plot_markers[i], markersize = MS, alpha = 0.5)
